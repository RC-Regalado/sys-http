...
- `readline_stream`: manejo confuso de `EAGAIN` con `||` mal planteado en el segundo `read` de refuerzo (inofensivo en la práctica porque `EAGAIN == EWOULDBLOCK`, pero no lo uses como ejemplo de buen estilo).
...
Corregidos (no reintroducir): `database.c` ya cierra `db_t` siempre y filtra por namespace; `Makefile clean` fixed; `db_selector_t.skip_blob` en `microdb` evita cargar blobs completos al listar/contar; **crítico** — `src/io.c` define `open`/`close`/`read`/`write` con nombre igual a libc y visibilidad global, lo que interponía sobre las llamadas equivalentes dentro de `libmicrodb.so` (rompía `db_open` en cualquier base de datos nueva) — corregido con `-fvisibility=hidden` en el `Makefile`, **no lo quites**; query string ya soportado (`query.c`, `cl->query`), ya no rompe el ruteo; **crítico** — `SIGSEGV` en `read_incoming` con requests reales de navegador (dos bugs independientes: overflow en `readline_stream` por no acotar el `read()` al espacio libre de `reader->buffer`, y NULL deref en `parse_header_line` cuando `cl->pool` se agotaba sin chequeo válido) — corregidos; `parse_headers` ahora sí valida que exista `:` antes de calcular longitudes; `CLIENT_BUF_SIZE` subido de 1024 a 8192 para tolerar cabeceras reales de navegador; **crítico** — el `Makefile` raíz no generaba dependencias de headers (`-MMD -MP`), así que un `make` incremental tras editar un `.h` compartido podía dejar `.o` compilados contra structs desactualizados (ABI mismatch entre unidades de traducción), causando descargas truncadas/con contenido incorrecto de forma intermitente — corregido agregando `-MMD -MP` + `-include $(DEP_FILES)`; **si ves contenido/tamaño incorrecto que no cuadra con la request, corré `make clean && make` antes de asumir un bug de lógica**; si después de eso un navegador real sigue mostrando el error (ej. `NS_ERROR_NET_RESET`) pero `curl`/un perfil de navegador limpio funcionan bien, **sospechá de una entrada de caché de disco corrupta en el perfil usado para probar** (le pasó exactamente eso al validar este fix) — probar en ventana privada/perfil nuevo antes de seguir investigando el servidor. Detalle completo: `docs/Architecture.md` ("Riesgo Crítico: SIGSEGV en `read_incoming`...", "Riesgo Crítico: Descargas truncadas/corruptas por objetos `.o` desactualizados...").
